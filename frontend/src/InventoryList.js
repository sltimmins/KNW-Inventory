...
export function InventoryItem(props) {
  ...
  return (
    <tr className="itemRow" onClick={(e) => handleRowClick(e)}>
      <th scope="row">{capitalize(props.listing.name)}</th>
...
export function InventoryList({ setListing, listings }) {
  ...
  listings.inventory.forEach(listing => {
    items.push(
      <InventoryItem key={listing.id} listing={listing} setListing={setListing} redraw={() => redraw()} />
    )
  })
  ...
  return (
    <div className="p-2 mt-5">
      <table className="table table-striped" ref={table} id="inventory">
